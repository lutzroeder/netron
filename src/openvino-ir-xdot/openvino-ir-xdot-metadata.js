class OpenVINOIRXdotOperatorMetadata {
    static open(host, callback) {
        if (!OpenVINOIRXdotOperatorMetadata.operatorMetadata) {
            OpenVINOIRXdotOperatorMetadata.operatorMetadata = new OpenVINOIRXdotOperatorMetadata();
        }
        callback(null, OpenVINOIRXdotOperatorMetadata.operatorMetadata);
    }
    ...
    static isEquivalent(a, b) {
        ...
        switch (className) {
            ...
            case '[object Array]':
                ...
                while (length--) {
                    if (!OpenVINOIRXdotOperatorMetadata.isEquivalent(a[length], b[length])) {
                        return false;
                    }
                }
                return true;
        ...
        while (size--) {
            var key = keys[size];
            if (!(b.hasOwnProperty(key) && OpenVINOIRXdotOperatorMetadata.isEquivalent(a[key], b[key]))) {
                return false;
            }
        }
        return true;
