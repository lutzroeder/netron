class OpenVINOIRXdotNode {
    constructor(layer, version, edges, layers) {
        ...
        _.each(layer.attr_list, ({name, value}) => {
            ...
            this._attributes.push(new OpenVINOIRXdotAttribute(this, name, value));
        });
    }
    ...
    get category() {
        return OpenVINOIRXdotOperatorMetadata.operatorMetadata.getOperatorCategory(this._type);
    }

    get documentation() {
        return OpenVINOIRXdotOperatorMetadata.operatorMetadata.getOperatorDocumentation(this._type);
    }
    ...
    get inputs() {
        ...
        const inputs = OpenVINOIRXdotOperatorMetadata.operatorMetadata.getInputs(this._type, list);
        return inputs.map((input) => {
            return new OpenVINOIRXdotArgument(input.name, input.connections.map((connection) => {
                if (connection.id instanceof CaffeTensor) {
                    return new OpenVINOIRConnection('', null, connection.id);
                }
                return new OpenVINOIRXdotConnection(connection.id, null, null);
            }));
        });
    }

    get outputs() {
        const outputs = OpenVINOIRXdotOperatorMetadata.operatorMetadata.getOutputs(this._type, this._outputs, this._name);
        return outputs.map((output) => {
            return new OpenVINOIRXdotArgument(output.name, output.connections.map((connection) => {
                return new OpenVINOIRXdotConnection(connection.id, null, null);
            }));
        });
    }
...
class OpenVINOIRXdotConnection {
...
class OpenVINOIRXdotAttribute {
    ...
    get visible() {
        const meta = OpenVINOIRXdotOperatorMetadata.operatorMetadata;
        return meta.getAttributeVisible(this._node.operator, this._name, this._value);
    }
}

class OpenVINOIRXdotTensor {
    constructor({data, shape, precision}) {
        this._data = data;
        this._shape = shape;
        const dataType = precision === 'FP32' ? 'float32' : '?';
        this._type = new OpenVINOIRXdotTensorType(dataType, this._shape);
    }
...
class OpenVINOIRXdotTensorType {
