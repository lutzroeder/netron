            ...
            case 7: {
                ...
                for (let i = 0; i < this.modules.length; i++) {
                    ...
                    for (let j = 0; j < functions.length; j++) {
                        ...
                        const inputs = new Array(function_header.inputs || 0);
                        ...
                        const outputs = new Array(function_header.outputs || 0);
                        for (let k = 0; k < outputs.length; k++) {
                            outputs[k] = reader.parameter(`output${j === 0 ? '' : (k + 1)}`);
                        }
                        for (let k = 0; k < outputs.length; k++) {
                        ...
                        switch (module_header.type) {
                            case 'stackvm': {
                                ...
                                if (reader) {
                                    ...
                                    layer.tensor_operations = layer.operations.filter((op) => op.opcode === 'tensor');
                                }
                                break;
                            }
                            case 'k210':
    ...
    read() {
        ...
        while (this._reader.position < this._reader.length) {
            ...
            if (!this._opcodes.has(opcode)) {
                throw new kmodel.Error(`Unknown opcode '${opcode}' at byte position ${position}.`);
            }
            const name = this._opcodes.get(opcode);
            const operation = { opcode: name, position };
            switch (name) {
                case 'ldc_i4':
                    operation.value = this._reader.int32();
                    break;
                case 'ldc_r4':
                    operation.value = this._reader.float32();
                    break;
                case 'lea_gp':
...
kmodel.BytecodeReader.v5 = class extends kmodel.BytecodeReader {
    ...
    constructor(buffer) {
        ...
        this._opcodes = new Map([
            ...
            [30, 'ldelem_u4'], [31, 'ldelem_u'], [32, 'ldelem_br2'], [33, 'ldelem_r4'],
            ...
            [46, 'ldarg_5'], [47, 'dup'], [48, 'pop'], [49, 'stshape'], [50, 'stpaddings'],
            ...
            [91, 'ecall'], [92, 'throw'], [93, 'break'], [94, 'tensor']
        ]);
    }
};

kmodel.BytecodeReader.v6 = class extends kmodel.BytecodeReader {
