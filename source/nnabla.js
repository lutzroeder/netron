...
nnabla.ModelFactory = class {
    ...
    match(context) {
        ...
        if (identifier.endsWith('.nnp'))
            return undefined;
        if (identifier.endsWith('.nntxt') || identifier.endsWith('.prototxt'))
            return 'nnabla.network';
        // if (identifier.endsWith('nnp_version.txt'))
        //     return 'nnabla.version';
        // if (identifier.endsWith('.protobuf'))
        //     return 'nnabla.parameter';

        return undefined;
    }

    open(context, match) {
        return context.require('./nnabla-proto').then(async () => {
            nnabla.proto = protobuf.get('nnabla').nnabla;
            const open = (model, version) => {
                return nnabla.Metadata.open(context).then((metadata) => {
            ...
            const get_version = () => {
                return context.request('nnp_version.txt', null).then((stream) => {
                    return String.fromCharCode.apply(null, stream.read(3));
                })
            }
            ...
            switch (match) {
                case 'nnabla.network': {
                    const model = get_model(context.stream);
                    const version = await get_version();
                    const params = await get_parameters();
...
nnabla.Tensor = class {
    ...
    constructor(name, type, values) {
        this._kind = "Initializer"; // TODO
        this._name = name;
        this._type = type;
        this._location = "default"; // TODO
        this._values = values;
    }
...
nnabla.Metadata = class {

    static open(context) {
        if (nnabla.Metadata._metadata) {
            return Promise.resolve(nnabla.Metadata._metadata);
        }
        return context.request('nnabla-metadata.json', 'utf-8', null).then((data) => {
            nnabla.Metadata._metadata = new nnabla.Metadata(data);
            return nnabla.Metadata._metadata;
        }).catch(() => {
            nnabla.Metadata._metadata = new nnabla.Metadata(null);
            return nnabla.Metadata._metadata;
        });
    }
