    ...
    match(context) {
        ...
        if (stream.length > 5) {
            ...
            if (buffer[0] === 0x08 && buffer[1] < 0x08 && buffer[2] === 0x12) {
                const producers = [ 'keras2onnx', 'tf2onnx', 'tflite2onnx', 'pytorch', 'skl2onnx', 'onnx-caffe2', 'OnnxMLTools', 'ML.NET', 'kneron_formatter', 'Kneron', 'kneron_kl530_test_case', 'onnxruntime-tools', 'onnx_test', 'CNTK', 'MATLAB Deep Learning Toolbox Converter for ONNX Model Format', 'MVTec Software' ];
